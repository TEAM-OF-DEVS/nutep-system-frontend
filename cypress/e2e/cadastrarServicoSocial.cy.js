describe("Formulário de Cadastro de Paciente", () => {
  ...
  it("Deve preencher e enviar o formulário de cadastro de paciente", () => {
    ...
    cy.get('input[name="descricaoConfiguracaoFamiliar"]').type(
      "Descrição Configuração familiar",
    );
    cy.get('select[name="situacaoConjugalPais"]').select("Casado (a)");
    cy.get('select[name="presencaDosPais"]').select(
      "Mãe e Pai convivem com a criança",
    );
    cy.get('select[name="tipoAcolhimento"]').select(
    ...
    cy.get('input[name="descricaoPeriodicidadeTerapia"]').type(
