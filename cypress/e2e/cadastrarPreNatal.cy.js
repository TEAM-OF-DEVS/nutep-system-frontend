describe('Formulário de Cadastro de Paciente', () => {
    ...
    it('Deve preencher e enviar o formulário de cadastro de paciente', () => {
        ...
        cy.get('input[name="descricaoProcedencia"]').type('Descrição da procedência.');
        ...
        cy.get('input[name="descricaoMetodoContraceptivo"]').type('Descrição do método contraceptivo.');
        //-----------------//
        cy.get('select[name="usoAborto"]').select('Sim');
        cy.get('input[name="tempoGestacaoSemanas"]').type('Tempo de gestação em semanas.');
        cy.get('select[name="meioAborto"]').select('Chás');
        cy.get('input[name="descricaoMeioAbortivo"]').type('Descrição do meio abortivo.');
        //-----------------//
        cy.get('select[name="consanguinidadePais"]').select('Sim');
        cy.get('input[name="descricaoConsanguinidade"]').type('Descrição da consanguinidade.');
        //-----------------//
        ...
        cy.get('input[name="descricaoDoencasPreExistentes"]').type('Descrição das doenças pré-existentes.');
        //-----------------//
        cy.get('select[name="usoDrogasMae"]').select('Nenhuma');
        cy.get('input[name="descricaoUsoDeDrogas"]').type('Descrição do uso de drogas.');
        //-----------------//
        cy.get('select[name="examesRealizadosMae"]').select('HIV - Normal');
        cy.get('input[name="descricaoExamesMae"]').type('Descrição dos exames realizados.');
        //-----------------//
        cy.get('select[name="medicamentosUtilizadosMae"]').select('Vitaminas');
        cy.get('input[name="descricaoMedicamentosMae"]').type('Descrição dos medicamentos utilizados.');
        //-----------------//
        ...
        cy.get('input[name="descricaoMotivoHospitalizacao"]').type('Descrição Motivo Hospitalar.');
        cy.get('select[name="diagnostico"]').select('Doenças autoimunes');
        cy.get('input[name="descricaoDiagnostico"]').type('Descrição do diagnóstico.');
